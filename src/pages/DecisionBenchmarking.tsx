...
const METRIC_LABELS: Record<string, { label: string; unit: string; lowerIsBetter?: boolean }> = {
  approvalRate: { label: "Genehmigungsrate", unit: "%" },
  avgDaysToDecision: { label: "Ø Entscheidungsdauer", unit: "Tage", lowerIsBetter: true },
  implementationRate: { label: "Umsetzungsrate", unit: "%" },
  overdueRate: { label: "Überfälligkeitsrate", unit: "%", lowerIsBetter: true },
  escalationRate: { label: "Eskalationsrate", unit: "%", lowerIsBetter: true },
  reviewCoverage: { label: "Review-Abdeckung", unit: "%" },
  riskMitigationRate: { label: "Risiko-Mitigation", unit: "%" },
  crossTeamCollaboration: { label: "Cross-Team-Zusammenarbeit", unit: "%" },
};
...
const DecisionBenchmarking = () => {
  ...
  const getOverallScore = () => {
    ...
    keys.forEach(key => {
      ...
      const meta = METRIC_LABELS[key];
      const range = meta.lowerIsBetter ? avg - hp : hp - avg;
      const diff = meta.lowerIsBetter ? avg - val : val - avg;
  ...
  const radarData = metrics
    ? Object.keys(METRIC_LABELS).map(key => {
        ...
        return {
          ...
          "Branchen-Ø": normalize(INDUSTRY_BENCHMARKS.average[key as MetricKey] as number),
          "Top 10%": normalize(INDUSTRY_BENCHMARKS.highPerformance[key as MetricKey] as number),
        };
      })
    : [];
  ...
  const barData = metrics
    ? Object.keys(METRIC_LABELS).map(key => ({
        ...
        Unternehmen: metrics[key],
        "Branchen-Ø": INDUSTRY_BENCHMARKS.average[key as MetricKey],
        "Top 10%": INDUSTRY_BENCHMARKS.highPerformance[key as MetricKey],
      }))
    : [];
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Card className="md:col-span-1">
                ...
                <CardContent className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-xs text-muted-foreground text-center">Score basiert auf dem Vergleich aller Metriken mit Branchen-Benchmarks</p>
                </CardContent>
              </Card>
                      ...
                      return (
                        <div key={key} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-sm font-semibold">{metrics[key]}{meta.unit === "%" ? "%" : ` ${meta.unit}`}</p>
                          </div>
                        </div>
                      );
                    })}
                    ...
                    <div className="h-[400px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <RadarChart data={radarData}>
                          ...
                          <Radar name="Ihr Unternehmen" dataKey="Ihr Unternehmen" stroke="hsl(var(--primary))" fill="hsl(var(--primary))" fillOpacity={0.3} />
                          <Radar name="Branchen-Ø" dataKey="Branchen-Ø" stroke="#f59e0b" fill="#f59e0b" fillOpacity={0.1} />
                          <Radar name="Top 10%" dataKey="Top 10%" stroke="#10b981" fill="#10b981" fillOpacity={0.1} />
                          <Legend />
                        </RadarChart>
                      </ResponsiveContainer>
                    </div>
                    ...
                    <div className="h-[400px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={barData} layout="vertical" margin={{ left: 120 }}>
                          ...
                          <Tooltip />
                          <Bar dataKey="Unternehmen" fill="hsl(var(--primary))" radius={[0, 4, 4, 0]} />
                          <Bar dataKey="Branchen-Ø" fill="#f59e0b" radius={[0, 4, 4, 0]} />
                          <Bar dataKey="Top 10%" fill="#10b981" radius={[0, 4, 4, 0]} />
                          <Legend />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b">
                            <th className="text-left py-2 px-3 font-medium text-muted-foreground">Metrik</th>
                            <th className="text-center py-2 px-3 font-medium text-muted-foreground">Ihr Wert</th>
                            <th className="text-center py-2 px-3 font-medium text-muted-foreground">Branchen-Ø</th>
                            <th className="text-center py-2 px-3 font-medium text-muted-foreground">Top 10%</th>
                            <th className="text-center py-2 px-3 font-medium text-muted-foreground">Bewertung</th>
                          </tr>
                        </thead>
                        <tbody>
                          {Object.entries(METRIC_LABELS).map(([key, meta]) => {
                            ...
                            return (
                              <tr key={key} className="border-b last:border-0">
                                <td className="py-2 px-3 font-medium">{meta.label}</td>
                                <td className="py-2 px-3 text-center font-semibold">{val}{suffix}</td>
                                <td className="py-2 px-3 text-center text-muted-foreground">{avg}{suffix}</td>
                                <td className="py-2 px-3 text-center text-muted-foreground">{hp}{suffix}</td>
            ...
            {/* Recommendations */}
            <Card>
              <CardHeader><CardTitle className="text-sm flex items-center gap-2"><Zap className="w-4 h-4" /> Empfehlungen</CardTitle></CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {Object.entries(METRIC_LABELS).map(([key, meta]) => {
                    ...
                    return (
                      <div key={key} className="p-3 rounded-lg border border-destructive/30 bg-destructive/5">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {gap > 0
                            ? `${gap}${meta.unit === "%" ? "pp" : ` ${meta.unit}`} Abstand zu Top-Performern. Fokus auf Prozessoptimierung empfohlen.`
                            : "Nahe am Benchmark – weiter optimieren."}
                        </p>
                      </div>
                    );
                  }).filter(Boolean)}
