...
import {
  ...
  Target, Users, Clock, BarChart3, Info,
} from "lucide-react";
...
const DecisionQualityIndex = () => {
  ...
  const { score, factors, trend, trendDelta, storyLine } = useMemo(() => {
    const decisions = isPersonal
      ? allDecisions.filter(d => d.created_by === user?.id || d.assignee_id === user?.id)
      : allDecisions;

    if (decisions.length < 2) {
      return { score: null, factors: [], trend: "neutral" as const, trendDelta: 0, storyLine: "" };
    }
    ...
    else if (totalScore >= 50) story = "Eure Entscheidungsqualität ist solide, mit Raum für Optimierung.";
    else if (totalScore >= 30) story = "Eure Entscheidungsqualität braucht Aufmerksamkeit.";
    else story = "Dringende Verbesserungen bei der Entscheidungsqualität nötig.";

    if (delta > 0) story += ` +${delta}% seit 30 Tagen.`;
    else if (delta < 0) story += ` ${delta}% seit 30 Tagen.`;

    return { score: Math.min(100, totalScore), factors, trend: t, trendDelta: delta, storyLine: story };
  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-xl border border-border bg-gradient-to-br ${getScoreGradient(score)} p-6 md:p-8`}
    >
