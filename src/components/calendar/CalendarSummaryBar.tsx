...
import { AlertTriangle, TrendingUp, DollarSign, Zap, Brain } from "lucide-react";
...
const CalendarSummaryBar = memo(({ decisions, tasks, currentDate, viewMode }: CalendarSummaryBarProps) => {
  const stats = useMemo(() => {
    ...
    const overdue = activeDecisions.filter(
    ...
    for (const d of activeDecisions) {
      ...
      if (daysOverdue > 0) {
        const mult = PRIORITY_MULTIPLIER[d.priority] ?? 1;
        totalDelayCost += daysOverdue * baseRate * mult;
      }
    }

    // AI insight: analyze next 14 days
    const next14 = addDays(new Date(), 14);
    const upcomingCritical = activeDecisions.filter(
      (d) =>
        d.due_date &&
        new Date(d.due_date) >= new Date() &&
        new Date(d.due_date) <= next14 &&
        (d.priority === "critical" || d.priority === "high")
    );

    let aiInsight = "";
    if (upcomingCritical.length >= 3) {
      const earliest = upcomingCritical.sort(
        (a: any, b: any) => new Date(a.due_date).getTime() - new Date(b.due_date).getTime()
      );
      const from = format(new Date(earliest[0].due_date), "d. MMM", { locale: de });
      const to = format(new Date(earliest[earliest.length - 1].due_date), "d. MMM", { locale: de });
      aiInsight = `Hohe Konzentration kritischer Entscheidungen (${upcomingCritical.length}) zwischen ${from} – ${to}. Kapazitätsplanung empfohlen.`;
    } else if (escalated > 0) {
      aiInsight = `${escalated} eskalierte Entscheidung${escalated > 1 ? "en" : ""} erfordern sofortige Aufmerksamkeit.`;
    } else if (overdue > 0) {
      aiInsight = `${overdue} überfällige Entscheidung${overdue > 1 ? "en" : ""} – Verzögerungskosten steigen.`;
    } else if (total > 0) {
      aiInsight = "Alle Entscheidungen im Plan. Keine kritischen Engpässe erkannt.";
    }

    const openTasks = (tasks ?? []).filter((t) => t.status !== "done").length;

    return { total, critical, escalated, overdue, totalDelayCost, aiInsight, openTasks };
  }, [decisions, tasks, currentDate]);

  const label = useMemo(() => {
    if (viewMode === "month") return format(currentDate, "MMMM yyyy", { locale: de });
    if (viewMode === "week") return "Diese Woche";
    return format(currentDate, "d. MMMM yyyy", { locale: de });
      ...
      <div className="flex flex-wrap items-center gap-3 px-4 py-3 rounded-xl border border-border bg-card">
        ...
        {stats.totalDelayCost > 0 && (
          <div className="flex items-center gap-1.5 text-xs ml-auto">
            ...
            <span className="font-semibold text-destructive">
              {stats.totalDelayCost.toLocaleString("de-DE")}€
            </span>
        ...
        {stats.openTasks > 0 && (
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>{stats.openTasks} offene Aufgaben</span>
          </div>
        )}
      </div>
      ...
      {stats.aiInsight && (
        <div className="flex items-start gap-2 px-4 py-2.5 rounded-lg border border-primary/20 bg-primary/5">
          ...
          <p className="text-xs text-foreground leading-relaxed">
            <span className="font-semibold text-primary">KI-Insight: </span>
            {stats.aiInsight}
          </p>
        </div>
      )}
