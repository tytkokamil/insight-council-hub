...
const UseCasesSection = () => (
  <section className="py-28 relative">
    <div id="use-cases" className="container mx-auto px-4">
      ...
      >
        <p className="text-xs font-medium text-muted-foreground/60 mb-4 tracking-[0.15em] uppercase">Use Cases</p>
        <h2 className="text-3xl md:text-4xl font-bold tracking-[-0.03em]">
          Gebaut für{" "}
          <span className="text-muted-foreground/50">Entscheidungsdruck</span>
        </h2>
      </motion.div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
        {useCases.map((useCase, i) => (
          ...
          >
            <useCase.icon className="w-5 h-5 text-muted-foreground/30 mb-4 group-hover:text-primary/60 transition-colors duration-500" />
            <h3 className="text-base font-semibold mb-2 tracking-tight">{useCase.title}</h3>
            <p className="text-sm text-muted-foreground/55 leading-relaxed">{useCase.description}</p>
          </motion.div>
        ))}
      </div>
    </div>
