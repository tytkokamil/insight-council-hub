...
const PricingAddons = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease }} className="mt-16 max-w-4xl mx-auto">
      <h3 className="text-xl font-bold text-center mb-2">{t("landing.pricing.addonsTitle")}</h3>
      <p className="text-sm text-muted-foreground text-center mb-8">{t("landing.pricing.addonsSubtitle")}</p>
      <div className="grid sm:grid-cols-2 gap-3">
        {addons.map((addon, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.04, duration: 0.4, ease }} className="flex items-start gap-3 p-4 rounded-xl border border-border/40 bg-card hover:border-foreground/10 transition-colors">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-sm font-semibold">{addon.name}</span>
                <span className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-muted text-muted-foreground">{addon.from}</span>
              </div>
