    ...
    >
      <div className={`mx-auto max-w-5xl px-4 transition-all duration-500 ${scrolled ? "mt-3" : "mt-4"}`}>
        <div className={`rounded-2xl transition-all duration-500 ${
          scrolled
            ? "bg-card/70 backdrop-blur-2xl border border-border/50 shadow-lg"
          ...
          <div className="px-5">
            <div className="flex items-center justify-between h-14">
              ...
              <div className="hidden md:flex items-center gap-1">
                {navItems.map(item => (
                  <a
                    key={item.label}
                    href={item.href}
                    className="text-[13px] text-muted-foreground hover:text-foreground px-3.5 py-1.5 rounded-lg transition-colors duration-300"
                  >
                    {item.label}
                  </a>
                ))}
              ...
              <div className="hidden md:flex items-center gap-2.5">
                <Link to="/login">
                  <Button variant="ghost" size="sm" className="rounded-lg text-muted-foreground hover:text-foreground text-[13px]">
                    Anmelden
                  </Button>
                </Link>
                <Link to="/dashboard">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="rounded-2xl bg-card/90 backdrop-blur-2xl border border-border/50 p-5 space-y-1 shadow-xl">
              {navItems.map(item => (
