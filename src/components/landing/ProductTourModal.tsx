...
import {
  ArrowRight, ArrowLeft, BarChart3, Brain, Users, Shield,
  Zap, GitBranch, CheckCircle2, TrendingUp, Clock, Target,
} from "lucide-react";
...
const tourSteps = [
  { title: "Entscheidungs-Dashboard", subtitle: "Alles auf einen Blick", description: "Verfolge alle Entscheidungen deines Teams in Echtzeit. Sehe Status, Prioritäten und Fortschritt auf einen Blick.", icon: BarChart3, preview: { type: "dashboard" as const } },
  ...
  <div className="space-y-3">
    <div className="flex gap-3">
      {[
        ...
        { label: "Genehmigt", value: "34" },
        { label: "Überfällig", value: "3" },
      ].map((s) => (
  ...
  <div className="space-y-3">
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="p-3 rounded-xl bg-muted/20 border border-border">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs font-semibold">KI-Risikoanalyse</span>
      </div>
...
const TeamPreview = () => (
  <div className="space-y-3">
    <div className="flex -space-x-2 mb-3">
      {["A", "B", "C", "D", "E"].map((l, i) => (
        <motion.div key={l} initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 + i * 0.1, type: "spring" }} className="w-8 h-8 rounded-full bg-foreground/[0.06] border-2 border-background flex items-center justify-center text-xs font-bold text-foreground/60">
          {l}
        </motion.div>
      ))}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="w-8 h-8 rounded-full bg-muted/30 border-2 border-background flex items-center justify-center text-xs text-muted-foreground">
        +8
      </motion.div>
    </div>
    {[
      ...
      <motion.div key={r.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.15 }} className="flex items-center justify-between p-2.5 rounded-xl bg-muted/20 border border-border/30">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-foreground/[0.06] flex items-center justify-center text-[10px] font-bold text-foreground/60">
            {r.name[0]}
          </div>
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
          r.status === "Approved" ? "bg-success/10 text-success"
            : r.status === "In Review" ? "bg-warning/10 text-warning"
  ...
  <div className="relative h-40">
    {[
      ...
      <motion.div key={node.label} initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 + i * 0.15, type: "spring" }} className="absolute" style={{ left: node.x, top: node.y }}>
        <div className="w-14 h-14 rounded-xl bg-foreground/[0.06] border border-border flex items-center justify-center">
          <span className="text-[10px] font-medium text-foreground/60">{node.label}</span>
        </div>
      </motion.div>
    ))}
    ...
    <Dialog open={open} onOpenChange={(v) => { onOpenChange(v); if (!v) setStep(0); }}>
      <DialogContent className="sm:max-w-xl p-0 gap-0 overflow-hidden border-border bg-card/95 backdrop-blur-2xl">
        ...
        <div className="px-6 pt-6 pb-4">
          ...
          <AnimatePresence mode="wait">
            <motion.p key={step} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-sm text-muted-foreground leading-relaxed">
              {current.description}
            </motion.p>
          </AnimatePresence>
        </div>
        ...
        <div className="px-6 pb-6 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={next} className="rounded-xl">
              {step < tourSteps.length - 1 ? (<>Weiter <ArrowRight className="w-4 h-4 ml-1" /></>) : "Jetzt starten"}
            </Button>
          </div>
        </div>
      </DialogContent>
