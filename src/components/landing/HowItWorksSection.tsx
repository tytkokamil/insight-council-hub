...
const HowItWorksSection = () => (
  <section className="py-28 relative overflow-hidden">
    ...
    <div className="container mx-auto px-4 relative z-10">
      ...
      >
        <p className="text-xs font-medium text-muted-foreground/50 mb-4 tracking-[0.15em] uppercase">So funktioniert's</p>
            ...
            >
              ...
              {i < 3 && (
                <div className="hidden md:block absolute top-10 right-0 w-full h-px">
                  <motion.div
                    className="h-full bg-gradient-to-r from-primary/20 to-primary/5"
                    initial={{ scaleX: 0 }}
              ...
              <div className="relative">
                <span className="text-5xl font-bold text-primary/[0.08] font-display block mb-4 group-hover:text-primary/[0.15] transition-colors duration-500">
                  {step.number}
                </span>
                <h3 className="text-lg font-semibold mb-2 tracking-tight">{step.title}</h3>
                <p className="text-sm text-muted-foreground/50 leading-relaxed">{step.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
