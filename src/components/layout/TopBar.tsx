...
const BREADCRUMB_MAP: Record<string, { area: string; areaKey: string }> = {
  ...
  "/benchmarking": { area: "intelligence", areaKey: "nav.intelligence" },
  "/engine": { area: "governance", areaKey: "nav.governance" },
  "/risks": { area: "governance", areaKey: "nav.governance" },
  "/automations": { area: "governance", areaKey: "nav.governance" },
  "/audit": { area: "governance", areaKey: "nav.governance" },
      ...
      <div className="flex items-center gap-1 min-w-0">
        {breadcrumb && (
          <nav className="flex items-center gap-1 text-[12px] text-muted-foreground" aria-label="Breadcrumb">
            <span>{t(breadcrumb.areaKey, { defaultValue: breadcrumb.area })}</span>
            {pageTitle && (
              <>
                <span className="opacity-40">›</span>
                <span className="text-foreground font-medium truncate">{t(pageTitle)}</span>
              </>
            )}
          </nav>
        )}
