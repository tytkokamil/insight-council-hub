...
const ApplyLearningPanel = ({
  ...
  const rankedLessons = useMemo(() => {
    ...
    return allLessons
      .map(l => {
        // Text similarity against takeaway + recommendations + decision title
        ...
        // Recency bonus (max 0.1 for last 30 days)
  ...
  // Auto-expand when good matches found
  useEffect(() => {
    if (rankedLessons.length > 0 && title.length > 5 && pinnedLessons.length === 0) {
      setExpanded(true);
    }
  }, [rankedLessons.length, title]);
  ...
  if (topLessons.length === 0 && pinnedLessons.length === 0) return null;
    ...
    <div className="pt-3 border-t border-border">
      ...
      {expanded && (
        <div className="mt-2 space-y-2 max-h-64 overflow-y-auto">
          ...
          {topLessons.length === 0 && pinnedLessons.length === 0 && (
