  ...
  const runCoPilot = async () => {
    ...
    try {
      ...
      const { data: allDecisions } = await supabase
        .from("decisions")
        .select("status, created_at, implemented_at, category")
      ...
      setResult(data);
      toast({ title: "Co-Pilot Analyse abgeschlossen" });
    } catch (e: any) {
      toast({ title: "Fehler", description: e.message || "Co-Pilot fehlgeschlagen", variant: "destructive" });
    }
    setLoading(false);
  };

  const impactColor = (v: string) =>
    v === "hoch" ? "text-success" : v === "mittel" ? "text-warning" : "text-muted-foreground";
  const priorityBadge = (v: string) =>
    v === "hoch" ? "bg-destructive/20 text-destructive" : v === "mittel" ? "bg-warning/20 text-warning" : "bg-muted text-muted-foreground";

  return (
    <div className="space-y-4 mt-4">
      <div className="flex justify-between items-center">
        <h3 className="text-sm font-medium flex items-center gap-1.5">
          <Compass className="w-4 h-4 text-primary" /> AI Decision Co-Pilot™
        </h3>
        <Button size="sm" onClick={runCoPilot} disabled={loading} className="gap-1 bg-gradient-to-r from-primary to-purple-500 hover:from-primary/90 hover:to-purple-500/90">
          ...
          {loading ? "Analysiere..." : "🧭 Co-Pilot starten"}
        </Button>
      </div>
      ...
      {result ? (
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-lg bg-primary/10 border border-primary/20">
            <p className="text-xs font-medium text-primary mb-1 flex items-center gap-1">
              <ArrowRight className="w-3 h-3" /> Nächster Schritt
            </p>
          ...
          <div className="p-4 rounded-lg bg-muted/30">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-medium flex items-center gap-1">
                <TrendingDown className="w-3 h-3 text-destructive" /> Ablehnungs-Wahrscheinlichkeit
              </p>
            ...
            <div className="w-full h-2 rounded-full bg-muted overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${result.rejection_probability > 50 ? "bg-destructive" : result.rejection_probability > 25 ? "bg-warning" : "bg-success"}`}
          ...
          <div className="p-4 rounded-lg bg-muted/30">
            <p className="text-xs font-medium flex items-center gap-1 mb-2">
              <UserCheck className="w-3 h-3 text-primary" /> Delegations-Empfehlung
            </p>
          ...
          <div className="p-4 rounded-lg bg-muted/30">
            <p className="text-xs font-medium flex items-center gap-1 mb-2">
              <Users2 className="w-3 h-3 text-primary" /> Reviewer-Empfehlungen
            </p>
          ...
          <div className="p-4 rounded-lg bg-muted/30">
            <p className="text-xs font-medium flex items-center gap-1 mb-2">
              <Zap className="w-3 h-3 text-primary" /> Prozess-Optimierungen
            </p>
            <div className="space-y-2">
                ...
                <div key={i} className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex gap-3 mt-1">
                      <span className={`text-[10px] ${impactColor(po.impact)}`}>Impact: {po.impact}</span>
                      <span className="text-[10px] text-muted-foreground">Aufwand: {po.effort}</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p className="text-sm">Starte den Co-Pilot für Delegations-, Reviewer- und Prozess-Empfehlungen.</p>
        </div>
      )}
    </div>
  );
