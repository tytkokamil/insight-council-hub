...
const positionConfig = {
  for: { label: "Dafür", icon: ThumbsUp, color: "text-success", bg: "bg-success/10 border-success/30" },
  against: { label: "Dagegen", icon: ThumbsDown, color: "text-destructive", bg: "bg-destructive/10 border-destructive/30" },
  neutral: { label: "Neutral", icon: Minus, color: "text-muted-foreground", bg: "bg-muted/30 border-border" },
};
...
const StakeholderAlignmentPanel = ({ decisionId }: { decisionId: string }) => {
  ...
  const alignmentScore = total > 0 ? Math.round((counts.for / total) * 100) : 0;
  const concerns = positions.filter(p => p.concerns).map(p => ({ name: p.profiles?.full_name || "Anonym", concern: p.concerns, position: p.position }));

  return (
    <div className="space-y-4 mt-4">
      <div className="flex items-center gap-2">
        <Users className="w-4 h-4 text-primary" />
        <h3 className="text-sm font-semibold">Stakeholder Alignment</h3>
        {total > 0 && (
          <span className="ml-auto text-xs px-2 py-0.5 rounded-full bg-muted font-medium">{total} Stimmen</span>
        )}
      </div>
      ...
      {total > 0 && (
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-xs text-muted-foreground">
            <span className="text-success">Dafür: {counts.for}</span>
            <span>Neutral: {counts.neutral}</span>
            <span className="text-destructive">Dagegen: {counts.against}</span>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2 pt-2 border-t border-border">
        <p className="text-xs font-medium">Deine Position:</p>
          ...
          {(Object.entries(positionConfig) as [string, any][]).map(([key, config]) => {
            ...
            return (
              <Button
                key={key}
                size="sm"
                variant={myPosition === key ? "default" : "outline"}
                className={`gap-1.5 text-xs flex-1 ${myPosition === key ? "" : ""}`}
                disabled={loading}
                onClick={() => submitPosition(key)}
              >
        ...
        <textarea
          ...
          placeholder="Deine Bedenken oder Anmerkungen (optional)..."
      ...
      {concerns.length > 0 && (
        <div className="space-y-2 pt-2 border-t border-border">
          <p className="text-xs font-medium flex items-center gap-1"><AlertCircle className="w-3 h-3" /> Bedenken & Anmerkungen</p>
          {concerns.map((c, i) => (
